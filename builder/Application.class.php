...
class Application extends \Forge\baseGenerator
{
    ...
    public function generateFolderStructure()
    {
        ...
        flush();
        print((mkdir(\Forge\Config::path("app") . '/' . $this->app_name, 0777) ? '.' : 'x'));
        flush();
        print((mkdir(\Forge\Config::path("app") . '/' . $this->app_name . '/config', 0777) ? '.' : 'x'));
        flush();
        print((mkdir(\Forge\Config::path("app") . '/' . $this->app_name . '/modules', 0777) ? '.' : 'x'));
        flush();
        print((mkdir(\Forge\Config::path("app") . '/' . $this->app_name . '/templates', 0777) ? '.' : 'x'));
        flush();
        print('DONE' . PHP_EOL);
        flush();
    }
